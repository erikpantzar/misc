...
function render(now, boost, sample) {
  ...
  const lit = Math.round(liveRatio * segments.length); const memoryEnd = Math.max(lit, Math.round(peakRatio * segments.length)); ledTrack.querySelectorAll('i').forEach((element, index) => { const zone = index < 8 ? 'green' : index < 13 ? 'amber' : 'red'; element.className = `${index < lit ? 'lit' : ''} ${index >= lit && index < memoryEnd ? 'memory' : ''} ${zone}`; });
